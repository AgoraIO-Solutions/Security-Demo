...
- (void)initializeAgoraEngine {
    AgoraRtcEngineConfig *config = [AgoraRtcEngineConfig alloc];
    config.appId = appID;
    config.areaCode = 0xFFFFFFFE;
    self.agoraKit = [AgoraRtcEngineKit sharedEngineWithConfig:config delegate:self];
    [self.agoraKit setEncryptionSecret:_encryptionPassword];
    [self.agoraKit setEncryptionMode:_encryptionMode];
}
...
- (void)joinChannel {
    int uid = [self getRandomNumber:1 to:99999];
    std::string result = agora::tools::RtcTokenBuilder::buildTokenWithUid([appID UTF8String], [certificate UTF8String], [_channel UTF8String], uid, agora::tools::UserRole::Role_Publisher);
    NSString *const token = [NSString stringWithCString:result.c_str() encoding:[NSString defaultCStringEncoding]];
    [self.agoraKit joinChannelByToken:token channelId:_channel info:nil uid:uid joinSuccess:^(NSString *channel, NSUInteger uid, NSInteger elapsed) {
